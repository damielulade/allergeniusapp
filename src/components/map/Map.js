...
function getMarkerColour(allergens, allergens_list, menu) {
  ...
  let total_safe_items = menu.filter(
    (item) => !total_unsafe_items.includes(item)
  );

  const percentage = parseInt(
    ((total_safe_items.length / menu.length) * 100).toFixed()
  );
...
export default function MapComponent() {
  ...
  useEffect(() => {
    const fetchUserData = () => {
      axios
        .get(`/api/current_filter/allergens`)
        .then((response) => {
    ...
    loader.load().then(() => {
      if (!mapInstanceRef.current) {
        ...
        markersData.forEach((markerData) => {
          const marker_colour = getMarkerColour(
            thisUserAllergens,
            markerData.allergens,
            markerData.menu
          );
